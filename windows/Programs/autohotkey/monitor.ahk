...
setMonitorInput(name) {
    setMonitorDdc("s " . name)
}
...
toggleGameMode() { ; Toggle between game/stream mode and reading mode. Game mode should utilize monitor's highest refresh rate. Stream mode should match resolution to TV's
    global displayMode
    if (displayMode == 3)
        displayMode := 4
    else if (displayMode == 4)
...
; }
; 
; toggleStreamMode() { ; Toggle between streaming mode and reading mode. Streaming mode has resolution matching TV's native.
;     global displayMode
;     if (displayMode != 4)
;         displayMode := 4
;     else
