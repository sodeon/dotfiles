function scale_or_zoom(delta)
    if (mp.get_property_bool("fullscreen", false)) then
        ...
        mp.set_property_number("current-window-scale", mp.get_property_number("current-window-scale") * (1.0 + delta))
        mp.osd_message("Scale: " .. mp.get_property_number("current-window-scale"))
    end
end
