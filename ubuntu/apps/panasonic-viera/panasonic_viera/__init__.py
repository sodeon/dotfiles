...
class RemoteControl:
    ...
    def get_apps(self):
        ...
        res = self.soap_request(URL_CONTROL_NRC, URN_REMOTE_CONTROL,
                                'X_GetAppList', None)
        root = etree.fromstring(res)
        el_apps = root.find('.//X_AppList')
        apps = el_apps.text.split("vc_app")[1:]
        app_list = {}
        for app in apps:
            prod_id = re.search('(?<=product_id\=)(.*?)\'', app).group(1)
            name = re.search('(?<='+prod_id+'\')(.*?)\'', app).group(1)
    ...
    def get_input_mode(self):
        """Return the input mode on the TV"""
        res = self.soap_request(URL_CONTROL_NRC, URN_REMOTE_CONTROL,
                                'X_GetInputMode', None)
        return res

    def set_input(self, arg):
        key = eval('Keys.' + arg).value
